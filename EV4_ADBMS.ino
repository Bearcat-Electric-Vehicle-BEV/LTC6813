...
void setup() {
    ...
    init_spi(CS1, SPI_MODE1); // SPI1 (ADC)
    init_can();
    ...
    if (mode == Init) {
        ...
        while (1) {
            ...
            if (determineMode(msg, CAN_baud_alt)) 
                break;
...
// Initialize SPI
void init_spi(uint8_t pin, uint8_t dataMode) {
    pinMode(pin, OUTPUT);
...
bool determineMode(CAN_message_t msg, bool CAN_baud_alt) {
...
// Precharge cycle loop 
void precharge_cycle(CAN_message_t msg, float charger_voltage, float charger_current) {
    while (1) {
        measure_voltage();
        measure_temp();
        reset_watchdog();
...
// Drive loop
void drive(int t, CAN_message_t msg) {
    while (1) {
        time_buffer[t] = millis() - start_time;
        if (t % current_interval == 0) {
            measure_current();
            current_buffer[int(t / current_interval)] = current;
        }
        ...
        if (t % volt_interval == 0) {
            ...
            for (int i = 0; i < num_boards; i++) {
                for (int j = 0; j < num_cells; j++) {
                    voltage_buffer[int(t / volt_interval)][i][j] = cell_voltage[i][j];
                }
            }
        }
        ...
        if (t % SD_interval == 0 && !memory_fault) 
            SD_data_write();
...
float update_SOC() {
    ...
    float min_OC_cell_voltage = open_circuit_voltage[0][0]; // funct. min_max requires that the min and
...
//   const int discharge_curve_length = sizeof(discharge_points) /
//   sizeof(discharge_points[0]);  //length of each discharge curve const float
//   max_capacity = discharge_points[0]; //maximum capacity of a single cell
//   const int num_current_curves = sizeof(discharge_currents) /
//   sizeof(discharge_currents[0]);  //number of discharge curves @ different
//   currents
// }
...
void wakeup_sleep(uint8_t total_ic) // Number of ICs in the system. This function
                                    ...
                                    // seconds of no command sent with valid PEC
{
